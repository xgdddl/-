...
function initCombatBarDrag($bar) {
  var drag = null;
  var hdr = $bar[0].querySelector('#cb-header');
  if (!hdr) return;
  hdr.addEventListener('mousedown', function(e){
    if (e.target.tagName === 'BUTTON') return;
    drag = {x: e.clientX - $bar[0].offsetLeft, y: e.clientY - $bar[0].offsetTop};
    $bar[0].style.transition = 'none';
    e.preventDefault();
  });
  document.addEventListener('mousemove', function(e){
    if (!drag) return;
    $bar[0].style.left = (e.clientX - drag.x) + 'px';
    $bar[0].style.top = (e.clientY - drag.y) + 'px';
    $bar[0].style.right = 'auto';
    $bar[0].style.bottom = 'auto';
  });
  document.addEventListener('mouseup', function(){
    if (drag) { drag = null; $bar[0].style.transition = ''; }
  });
  // Touch support
  hdr.addEventListener('touchstart', function(e){
    if (e.target.tagName === 'BUTTON') return;
    var t = e.touches[0];
    drag = {x: t.clientX - $bar[0].offsetLeft, y: t.clientY - $bar[0].offsetTop};
    $bar[0].style.transition = 'none';
  }, {passive:false});
  document.addEventListener('touchmove', function(e){
    if (!drag) return;
    var t = e.touches[0];
    $bar[0].style.left = (t.clientX - drag.x) + 'px';
    $bar[0].style.top = (t.clientY - drag.y) + 'px';
    $bar[0].style.right = 'auto';
    $bar[0].style.bottom = 'auto';
  }, {passive:false});

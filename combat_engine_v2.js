...
function collectEditorData() {
  var u = {
    ...
    hp: parseInt($("#ue-hp").val()), maxhp: parseInt($("#ue-maxhp").val())||10,
...
function buildCombatBar() {
  ...
  var cls = COMBAT_BAR_COLLAPSED ? ' cb-collapsed' : '';
  var hdrText = active ? ('⚔️ ' + (data.battleName||"战斗") + ' · 回合 ' + data.round) : '⚔️ 战斗引擎 (待机)';
  ...
  html += '</div>';
  html += '<button id="cb-collapse-btn">' + (COMBAT_BAR_COLLAPSED ? '▲' : '▼') + '</button>';
  html += '</div>';
  ...
  html += '<input id="cb-action-input" type="text" placeholder="输入战斗动作 (如: 用火焰箭攻击哥布林A)..." maxlength="300" style="flex:1;padding:8px 10px;background:#1a1a2e;border:1px solid #444;color:#e0d0b0;border-radius:4px;font-size:12px;">';
  html += '<button id="cb-btn-add-unit" style="padding:6px 10px;background:#16a34a;color:#fff;border:none;border-radius:3px;cursor:pointer;font-size:11px;">+单位</button>';
  html += '<button id="cb-next-round" style="padding:8px 14px;background:#2563eb;color:#fff;border:none;border-radius:4px;cursor:pointer;font-size:12px;font-weight:600;">下一回合 ▶</button>';
  html += '<button id="cb-end-combat" style="padding:8px 14px;background:#dc2626;color:#fff;border:none;border-radius:4px;cursor:pointer;font-size:12px;font-weight:600;">终止 ⏹</button>';
  html += '</div>';
...
function buildCharsTabContent(data) {
  if (!data || !data.units || data.units.length === 0) {
    return '<div style="color:#888;padding:10px;text-align:center;">暂无单位 · 点击 "+单位" 添加</div>';
  }
  var html = '';
  data.units.forEach(function(u) {
    if (u.hp <= 0) {
      html += '<div style="opacity:0.4;padding:6px 8px;border-bottom:1px solid #333;">💀 ' + u.name + ' (阵亡)</div>';
      return;
    }
    var color = u.side === "ally" ? "#4ade80" : (u.side === "enemy" ? "#f87171" : "#fbbf24");
    var hpPct = Math.round(u.hp / u.maxhp * 100);
    var barColor = hpPct > 50 ? "#4ade80" : hpPct > 25 ? "#fbbf24" : "#ef4444";
    html += '<div class="cb-unit-card" data-unit="' + u.name + '" style="padding:6px 8px;border-bottom:1px solid #333;cursor:pointer;display:flex;gap:8px;align-items:center;">';
    // 头像
    html += '<div style="width:36px;height:36px;border-radius:50%;background:#2a2a3a;border:2px solid ' + color + ';flex-shrink:0;overflow:hidden;display:flex;align-items:center;justify-content:center;">';
    if (u.avatar) {
      html += '<img src="' + u.avatar + '" style="width:100%;height:100%;object-fit:cover;">';
    } else {
      html += '<span style="font-size:14px;color:' + color + ';font-weight:700;">' + (u.name.replace(/[a-zA-Z]/g,'').slice(0,2) || u.name.slice(0,1)) + '</span>';
    }
    html += '</div>';
    // 信息
    html += '<div style="flex:1;min-width:0;">';
    html += '<div style="display:flex;justify-content:space-between;align-items:center;">';
    html += '<span style="color:' + color + ';font-weight:600;">' + u.name + '</span>';
    html += '<span style="font-size:10px;color:#888;">Lv.' + (u.level||1) + ' AC' + (u.ac||10) + ' <button class="cb-edit-unit" data-unit="'+u.name+'" style="padding:1px 6px;background:#333;color:#aaa;border:1px solid #555;border-radius:2px;cursor:pointer;font-size:10px;">✎</button></span>';
    html += '</div>';
    html += '<div style="display:flex;align-items:center;gap:6px;margin-top:3px;">';
    html += '<div style="flex:1;background:#333;border-radius:3px;height:6px;overflow:hidden;">';
    html += '<div style="width:' + hpPct + '%;height:100%;background:' + barColor + ';border-radius:3px;"></div>';
    html += '</div>';
    html += '<span style="font-size:10px;">' + u.hp + '/' + u.maxhp + '</span>';
    html += '</div>';
    html += '<div style="font-size:10px;color:#888;margin-top:1px;">力'+(u.attr.str||10)+' 敏'+(u.attr.dex||10)+' 体'+(u.attr.con||10)+' 智'+(u.attr.int||10)+' 感'+(u.attr.wis||10)+' 魅'+(u.attr.cha||10);
    if (u.status && u.status.length > 0) html += ' <span style="color:#fbbf24;">[' + u.status.join(',') + ']</span>';
    html += '</div>';
    html += '</div>'; // info
    html += '</div>'; // card
  });
...
function buildMapTabContent(data) {
  if (!data || !data.units || data.units.length === 0) {
    return '<div style="color:#888;padding:20px;text-align:center;">暂无单位 · 先在「角色」Tab添加单位</div>';
  }
  ...
  html += '<button class="cb-map-zoom" data-dir="out" style="padding:2px 8px;background:#333;color:#ccc;border:1px solid #555;border-radius:3px;cursor:pointer;">−</button>';
  html += '<span style="color:#888;">' + Math.round(COMBAT_MAP_SCALE*100) + '%</span>';
  html += '<button class="cb-map-zoom" data-dir="in" style="padding:2px 8px;background:#333;color:#ccc;border:1px solid #555;border-radius:3px;cursor:pointer;">+</button>';
  html += '<button class="cb-map-fit" style="padding:2px 8px;background:#333;color:#ccc;border:1px solid #555;border-radius:3px;cursor:pointer;font-size:10px;">适应</button>';
  html += '<span style="color:#888;margin-left:auto;">' + gs.cols + '×' + gs.rows + ' · 拖拽单位放置</span>';
  html += '</div>';

  // 网格容器
  html += '<div id="cb-map-viewport" style="overflow:auto;max-height:280px;border:1px solid #333;border-radius:4px;position:relative;background:#1a1a24;">';
  html += '<div id="cb-map-grid" style="width:' + (gridW * COMBAT_MAP_SCALE) + 'px;height:' + (gridH * COMBAT_MAP_SCALE) + 'px;position:relative;background-image:linear-gradient(rgba(255,255,255,0.06) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,0.06) 1px,transparent 1px);background-size:' + (gs.cellSize * COMBAT_MAP_SCALE) + 'px ' + (gs.cellSize * COMBAT_MAP_SCALE) + 'px;transform-origin:0 0;">';
  ...
  aliveUnits.forEach(function(u){
    ...
    var left = pos.col * cs + (cs - 36) / 2;
    var top = pos.row * cs + (cs - 36) / 2;
    var color = u.side === 'ally' ? '#4ade80' : (u.side === 'enemy' ? '#f87171' : '#fbbf24');
    var borderColor = COMBAT_SELECTED_UNIT === u.name ? '#fff' : color;
    var hpPct = Math.round(u.hp / u.maxhp * 100);
    var initials = u.name.replace(/[a-zA-Z]/g,'').slice(0,2) || u.name.slice(0,2);

    html += '<div class="cb-map-token" data-unit="' + u.name + '" ';
    html += 'style="position:absolute;left:' + left + 'px;top:' + top + 'px;';
    html += 'width:36px;height:36px;border-radius:50%;';
    html += 'background:#1e1e2e;border:2px solid ' + borderColor + ';';
    html += 'display:flex;align-items:center;justify-content:center;';
    html += 'cursor:grab;z-index:10;overflow:hidden;';
    html += 'box-shadow:0 0 6px ' + color + '44;"';
    html += 'title="' + u.name + ' HP' + u.hp + '/' + u.maxhp + '">';
    if (u.avatar) {
      html += '<img src="' + u.avatar + '" style="width:100%;height:100%;object-fit:cover;position:absolute;top:0;left:0;opacity:0.8;">';
    }
    html += '<span style="font-size:10px;font-weight:700;color:' + color + ';position:relative;z-index:1;text-shadow:0 1px 2px #000;">' + initials + '</span>';
    // HP环
    html += '<div style="position:absolute;bottom:-2px;left:2px;right:2px;height:3px;background:#333;border-radius:2px;"><div style="height:100%;width:' + hpPct + '%;background:' + (hpPct>50?'#4ade80':hpPct>25?'#fbbf24':'#ef4444') + ';border-radius:2px;"></div></div>';
    html += '</div>';
  });

  // 攻击箭头
  COMBAT_MAP_ARROWS.forEach(function(arr, idx){
    ...
    html += drawArrowSVG(x1, y1, x2, y2, arr.color || '#fbbf24', arr.damage || '', idx);
  });

  html += '</div>'; // #cb-map-grid
  html += '</div>'; // #cb-map-viewport
  html += '<div style="font-size:10px;color:#666;padding:2px 8px;">💡 拖拽单位到格子上 · 点单位选中 · 攻击后自动画箭头</div>';
  return html;
}
...
function mountCombatBar() {
  $("#combat-bar, #combat-bar-css").remove();

  $('<style id="combat-bar-css">').text([
    '#combat-bar { position:fixed; bottom:0; left:0; right:0; z-index:90000; background:#12121e; border-top:1px solid #333; font-size:12px; color:#d0c8b8; display:flex; flex-direction:column; max-height:50vh; transition:max-height 0.2s; }',
    '#combat-bar.cb-collapsed #cb-body, #combat-bar.cb-collapsed #cb-input-row { display:none; }',
    '#combat-bar.cb-collapsed { max-height:32px; }',
    '#cb-header { display:flex; align-items:center; padding:4px 6px; background:#1a1a2e; cursor:pointer; user-select:none; min-height:28px; gap:4px; flex-wrap:wrap; }',
    '#cb-title { font-weight:600; font-size:11px; white-space:nowrap; color:#e0d0b0; }',
    '#cb-tab-bar { display:flex; gap:1px; flex:1; justify-content:center; flex-wrap:wrap; }',
    '.cb-tab { padding:3px 8px; border:none; background:transparent; color:#888; cursor:pointer; font-size:10px; border-radius:3px; transition:all 0.15s; white-space:nowrap; }',
    '.cb-tab:hover { background:#2a2a3a; color:#ccc; }',
    '.cb-tab.active { background:#2563eb; color:#fff; }',
    '#cb-collapse-btn { padding:2px 6px; border:none; background:transparent; color:#888; cursor:pointer; font-size:12px; }',
    '#cb-body { flex:1; overflow-y:auto; max-height:35vh; padding:4px 6px; }',
    '.cb-tab-panel { display:block; }',
    '.cb-tab-panel.cb-hidden { display:none !important; }',
    '#cb-input-row { display:flex; gap:4px; padding:6px; background:#1a1a2e; border-top:1px solid #222; align-items:center; flex-wrap:wrap; }',
    '.cb-btn-active { background:#2563eb !important; color:#fff !important; border-color:#2563eb !important; }',
    // 地图 Token
    '.cb-map-token { transition: left 0.3s ease, top 0.3s ease, transform 0.15s, box-shadow 0.15s; }',
    '.cb-map-token:hover { transform: scale(1.25); z-index:20 !important; box-shadow: 0 0 12px rgba(255,255,255,0.3) !important; }',
    '.cb-map-token.cb-selected { border-color:#fff !important; box-shadow: 0 0 14px rgba(255,255,255,0.5) !important; animation: cb-pulse 1.2s ease-in-out infinite; }',
    '@keyframes cb-pulse { 0%,100% { box-shadow: 0 0 8px rgba(255,255,255,0.3); } 50% { box-shadow: 0 0 18px rgba(255,255,255,0.7); } }',
    // 移动端适配
    '@media (max-width: 768px) {',
    '  #combat-bar { max-height:60vh; font-size:11px; }',
    '  #cb-title { font-size:10px; }',
    '  #cb-tab-bar { gap:0; }',
    '  .cb-tab { padding:3px 5px; font-size:9px; border-radius:2px; }',
    '  #cb-body { max-height:30vh; padding:2px 4px; }',
    '  #cb-input-row { gap:3px; padding:4px; }',
    '  #cb-input-row input { font-size:11px; padding:6px 8px; }',
    '  #cb-input-row button { font-size:10px; padding:6px 8px; }',
    '  #cb-map-viewport { max-height:180px; }',
    '  .cb-map-token { width:28px !important; height:28px !important; }',
    '  .cb-unit-card { padding:4px 6px !important; }',
    '  #cb-collapse-btn { padding:4px 10px; }',
    '}',
    '@media (max-width: 480px) {',
    '  #combat-bar { max-height:70vh; }',
    '  #cb-header { padding:3px 4px; gap:2px; }',
    '  #cb-title { font-size:9px; }',
    '  .cb-tab { padding:2px 4px; font-size:8px; }',
    '  #cb-body { max-height:25vh; }',
    '  #cb-input-row { gap:2px; }',
    '  #cb-input-row input { flex:1 1 100%; min-width:0; }',
    '  #cb-input-row button { flex:1 1 auto; font-size:9px; padding:5px 6px; }',
    '  #cb-map-viewport { max-height:140px; }',
    '  .cb-map-token { width:24px !important; height:24px !important; }',
    '}',
  ].join('\n')).appendTo('head');
...
function bindCombatBarEvents() {
  ...
  $("#cb-collapse-btn").off("click").on("click", function(){
    ...
    $("#combat-bar").toggleClass("cb-collapsed", COMBAT_BAR_COLLAPSED);
...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   PART 3.5: 友方数据读取 (ZOD → 正文回退)
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */

function fetchAllyFromZOD() {
  var allies = [];
  try {
    // 尝试读取主角面板 (ZOD 变量 key: 主角面板)
    var playerData = TavernHelper.getVariables("主角面板");
    if (playerData) {
      allies.push({
        name: playerData.姓名 || "主角",
        side: "ally",
        type: "player",
        level: playerData.等级 || 1,
        template: playerData.模板 || "普通",
        hp: playerData.HP ? playerData.HP.当前 : undefined,
        maxhp: playerData.HP ? playerData.HP.最大 : undefined,
        mp: playerData.MP ? playerData.MP.当前 : 0,
        maxmp: playerData.MP ? playerData.MP.最大 : 0,
        ac: playerData.护甲值 || 10,
        attr: {
          str: (playerData.属性 && playerData.属性.力量) || 10,
          dex: (playerData.属性 && playerData.属性.敏捷) || 10,
          con: (playerData.属性 && playerData.属性.体质) || 10,
          int: (playerData.属性 && playerData.属性.智力) || 10,
          wis: (playerData.属性 && playerData.属性.感知) || 10,
          cha: (playerData.属性 && playerData.属性.魅力) || 10,
        },
        skills: playerData.技能 || {},
        status: [],
        swarm: null,
      });
    }
  } catch(e) {
    console.warn("[Combat] ZOD主角数据读取失败:", e);
  }

  // 尝试读取NPC/队友数据
  try {
    // ZOD可能以 NPC数据 或 npc_data 为key存储
    var npcData = TavernHelper.getVariables("NPC数据");
    if (npcData && typeof npcData === "object") {
      for (var npcName in npcData) {
        if (npcData.hasOwnProperty(npcName)) {
          var npc = npcData[npcName];
          allies.push({
            name: npc.姓名 || npcName,
            side: "ally",
            type: "npc",
            level: npc.等级 || 1,
            template: npc.模板 || "普通",
            hp: npc.HP ? npc.HP.当前 : undefined,
            maxhp: npc.HP ? npc.HP.最大 : undefined,
            mp: npc.MP ? npc.MP.当前 : 0,
            maxmp: npc.MP ? npc.MP.最大 : 0,
            ac: npc.护甲值 || 10,
            attr: {
              str: (npc.属性 && npc.属性.力量) || 10,
              dex: (npc.属性 && npc.属性.敏捷) || 10,
              con: (npc.属性 && npc.属性.体质) || 10,
              int: (npc.属性 && npc.属性.智力) || 10,
              wis: (npc.属性 && npc.属性.感知) || 10,
              cha: (npc.属性 && npc.属性.魅力) || 10,
            },
            skills: npc.技能 || {},
            status: [],
            swarm: null,
    ...
    console.warn("[Combat] ZOD NPC数据读取失败:", e);
  }
...
async function handleInitFromChat() {
  try {
    ...
    // 从ZOD读取友方数据 (主角+队友)
    var allyUnits = fetchAllyFromZOD();
    if (allyUnits.length === 0) {
      // ZOD没有 → 尝试从聊天历史解析
      console.warn("[Combat] ZOD无友方数据，需要手动初始化主角。");
    }

    // 合并: ZOD友方 + AI识别的敌方(+额外友方NPC)
    var allUnits = allyUnits.slice();
    parsedUnits.forEach(function(u) {
      // 避免重复添加(已存在于ZOD中的单位)
    ...
    toastr.success("战斗初始化完成，" + allUnits.length + "个单位 (" + allyUnits.length + "友方 + " + (allUnits.length - allyUnits.length) + "敌方)");
    refreshCombatBar();
  } catch(e) {

...
function initCombatBarDrag($bar) {
  ...
  $bar.find('#cb-header').off('mousedown.drag touchstart.drag').on('mousedown.drag touchstart.drag', function(e){
    if ($(e.target).is('button')) return;
    drag.active = true;
    var ev = e.originalEvent && e.originalEvent.touches ? e.originalEvent.touches[0] : e;
    drag.x = ev.clientX - $bar.offset().left;
    drag.y = ev.clientY - $bar.offset().top;
    $bar.css({transition:'none'});
  });
  $(document).off('mousemove.drag touchmove.drag').on('mousemove.drag touchmove.drag', function(e){
    if (!drag.active) return;
    var ev = e.originalEvent && e.originalEvent.touches ? e.originalEvent.touches[0] : e;
    $bar.css({left:ev.clientX-drag.x, top:ev.clientY-drag.y, right:'auto', bottom:'auto'});
  });
  $(document).off('mouseup.drag touchend.drag').on('mouseup.drag touchend.drag', function(){
    drag.active = false;
    $bar.css({transition:''});
  });
}
